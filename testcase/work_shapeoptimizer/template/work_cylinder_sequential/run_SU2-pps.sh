...
source ${HOME}/.bashrc_precice
source ${HOME}/.bashrc_precice_su2
source ${HOME}/.bashrc_precice_borealis
...
# Optimization
PYHON=python3
LD_O=${HOME_BORE}/src_pilot/adapter_shapeoptimizer/shapeoptimizer_preCICE_pilot.py
...
if ! $fluid ; then # Coupled
  if $parallel ; then
    $MPIP -np $ncpu_f ${PYHON} ${LD_F} -f ${INP_F} -d 2 --parallel > $LOG_F 2>&1 &
  else
