...
class adapter_shapeoptimizer(orbital):
  ...
  def initial_settings(self, config):
    ...
    # Note: ここのPSOはよくないかも
    num_process = config['PSO'].get("num_process", cpu_count())
    ...
    # Caseディレクトリの作成
    for n in range(0,num_process):
      work_dir_case = self.work_dir + '/' + self.case_dir + '_rank' + str(n+1).zfill(self.step_digit)
    ...
    subprocess.run(["mpirun.openmpi","--version"])
  ...
  def objective_function(self, parameter_opt, *args):
    ...
    work_dir_case = self.work_dir + '/' + self.case_dir + '_rank' + str(particle_id).zfill(self.step_digit)
    ...
    if self.step == 0:
      ...
      #self.run_subprocess()
#      subprocess.Popen(["bash", "run_SU2-OPT_pps.sh"], cwd=work_dir_case )
      subprocess.call(["./run_SU2-OPT_pps.sh"], cwd=work_dir_case )
